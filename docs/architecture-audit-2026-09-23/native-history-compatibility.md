...
## Pre-dispatch projection ownership

Real diagnostic evidence identified duplicate historical output at the native-to-EventStore hydration writer, not a text-rendering defect: a recovered Rust-finalized partial and its newly materialized native row have different local ids. A suffix merge places that second representation after the prepared user. Changed-prefix hydration now installs the verified native projection plus existing local failed-delivery/retry-lineage sidecars and the exact prepared user last, before starting the provider. Distinct equal native messages remain distinct. Unchanged prefixes skip all I/O. Covered layers: production call chain, identity/authority, preparation lifecycle, source projections and init parity with newly materialized episodes. Wire/schema, catalog/auth and platform launch policies are unchanged. Rollback is a source revert; no raw data cleanup or migration. Four hydration cases fail appropriately on the former implementation and pass with continuation/settled-tail tests (108 total). Large-prefix replacement cost and corrected GUI acceptance remain unverified.
