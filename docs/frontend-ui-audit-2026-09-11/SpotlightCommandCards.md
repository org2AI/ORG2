...
Scope: GUI/TUI presentation for the main command list; other palettes retain their existing controls.
...
| `src/scaffold/GlobalSpotlight/components/SpotlightCardList.tsx:12`    | Fixed card height  | keep with reason | One local geometry constant coordinates card height and virtual row estimates; compact 80px cards use single-line labels with full-title tooltips   | None             |
| `src/scaffold/GlobalSpotlight/components/SpotlightItemRow.tsx:401`    | Card appearance    | keep with reason | Existing border, background, text, radius and shadow tokens support themes; the two-column inner grid places icon/shortcut above the label          | None             |
