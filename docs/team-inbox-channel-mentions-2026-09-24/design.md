...
Performance verdict: blocked for full memory acceptance. The [real Realtime/native run](realtime-acceptance.md) now proves automatic bidirectional notification, same-account receipts, disconnect recovery, hidden/foreground catch-up, automatic private-detail eviction, bounded initial loading under a 1,000-message burst and quiet idle/hidden windows. Repeated-cycle measurements found several-hundred-MiB retained WebKit allocations requiring JS/DOM ownership profiling. This is not a proven linear leak or a completed large Share Sessions performance investigation. Physical two-machine behavior, OS notifications and hosted auth refresh are not claimed. The fixes retain one selection key and one scoped action error; they introduce no timer, listener, retry loop or growing cache.
