...
The lifecycle now owns the Promise, retries within the existing bounded ladder, retains live events, and reports terminal provider failures immediately and exhausted read failures through the active session's runtime error. It checks session and turn generation after asynchronous work so a stale read cannot update a different session or newer turn. Its own synchronous post-load lifecycle transition is carried into the next retry. Canonical full-history reads and missing-file errors are unchanged: an unavailable native transcript is never certified as an empty export.
