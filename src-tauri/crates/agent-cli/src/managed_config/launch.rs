...
pub fn prepare_with_proxy_token(
    ...
    write_profile(agent, model, session_id, &url, token, false, None)
}
...
pub fn restore_with_proxy_token(
    ...
    let _guard = config_operation_guard()?;
    write_profile(agent, model, session_id, url, token, true, None)
}

pub fn restore_with_auth_command(
    agent: &str,
    model: &str,
    session_id: &str,
    url: &str,
    command: &super::command_auth::CredentialCommand,
) -> Result<ManagedLaunchProfile, String> {
    let _guard = config_operation_guard()?;
    write_profile(agent, model, session_id, url, "", true, Some(command))
}
...
fn write_profile(
    ...
    token: &str,
    restore: bool,
    auth: Option<&super::command_auth::CredentialCommand>,
) -> Result<ManagedLaunchProfile, String> {
    ...
    let mut content = match auth {
        Some(command) => super::command_auth::generate(agent, model, url, command)?,
        None => content,
    };
