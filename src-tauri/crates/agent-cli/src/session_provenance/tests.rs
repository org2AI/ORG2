...
#[test]
fn isolated_json_profile_gets_managed_hooks_without_losing_user_settings() {
    ...
    update_json_platform_at_path(
        SessionProvenanceHookPlatform::ClaudeCode,
        &path,
        true,
        true,
        Path::new("/opt/orgii/bin/orgii"),
    )
    .expect("project isolated hooks");

    let projected: Value =
        serde_json::from_slice(&std::fs::read(&path).expect("projected settings"))
            .expect("projected json");
    assert_eq!(projected["theme"], "dark");
    assert_eq!(
        projected["hooks"]["Stop"][0]["hooks"][0]["command"],
        "user-stop-hook"
    );
    assert!(config_has_complete_managed_hooks(
        &projected,
...
#[test]
fn isolated_cursor_and_codex_hook_files_use_their_native_json_shapes() {
    for platform in [
        SessionProvenanceHookPlatform::Cursor,
        SessionProvenanceHookPlatform::Codex,
    ] {
        ...
        let projected: Value =
            serde_json::from_slice(&std::fs::read(&path).expect("read isolated hooks"))
                .expect("isolated hooks json");
        assert!(
            config_has_complete_managed_hooks(&projected, platform, true),
            "{platform:?} must receive its complete native managed-hook shape"
        );
    }
}
