...
pub(crate) fn create_schema(conn: &Connection) -> SqliteResult<()> {
    ...
    let schema = format!(
        "CREATE TABLE IF NOT EXISTS agent_org_execution_inbox_materializations (
        ...
        CREATE TABLE IF NOT EXISTS agent_org_execution_inbox_delivery_resolutions (
            inbox_id INTEGER PRIMARY KEY,
        ...
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_delivery_resolutions_run
            ON agent_org_execution_inbox_delivery_resolutions(org_run_id, inbox_id);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_materializations_session
            ON agent_org_execution_inbox_materializations(session_id, inbox_id);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_recipient_member_unread
            ON agent_org_execution_inbox(recipient_member_id, read_at, created_at);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_recipient_unread
            ON agent_org_execution_inbox(recipient_agent_id, read_at, created_at);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_org_run
            ON agent_org_execution_inbox(org_run_id, created_at);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_org_run_id
            ON agent_org_execution_inbox(org_run_id, id);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_run_unread_recipient
            ON agent_org_execution_inbox(org_run_id, recipient_member_id, recipient_agent_id, id)
            WHERE read_at IS NULL;
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_run_kind_id
            ON agent_org_execution_inbox(org_run_id, payload_kind, id);
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_run_task_assignment_v4
            ON agent_org_execution_inbox(
                org_run_id,
                recipient_member_id,
                json_extract(
        ...
        DROP INDEX IF EXISTS idx_agent_org_execution_inbox_run_task_assignment_v3;
        DROP INDEX IF EXISTS idx_agent_org_execution_inbox_run_task_assignment_v2;
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_request_id
            ON agent_org_execution_inbox(request_id);
        CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_user_message_once
            ON agent_org_execution_inbox(org_run_id, sender_agent_id, client_message_id)
            WHERE client_message_id IS NOT NULL;
        DROP INDEX IF EXISTS idx_agent_org_execution_inbox_causation_once;
        CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_org_execution_inbox_causation_recipient_once
            ON agent_org_execution_inbox(
                causation_inbox_id,
                payload_kind,
                recipient_agent_id,
...
pub(crate) fn repair_dangling_materializations(conn: &Connection) -> SqliteResult<()> {
    ...
    if transcript_tables_exist {
        conn.execute(
            "DELETE FROM agent_org_execution_inbox_materializations AS receipt
             WHERE NOT EXISTS (
...
fn create_agent_inbox_table(conn: &Connection) -> SqliteResult<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS agent_org_execution_inbox (
...
mod tests {
    ...
    fn canonical_schema_contains_current_columns_and_indexes() {
        ...
        let mut stmt = conn
            .prepare("PRAGMA table_info(agent_org_execution_inbox)")
        ...
        assert!(conn
            .query_row(
                "SELECT EXISTS(
                     ...
                     WHERE type='table'
                       AND name='agent_org_execution_inbox_task_bindings'
                 )",
                [],
