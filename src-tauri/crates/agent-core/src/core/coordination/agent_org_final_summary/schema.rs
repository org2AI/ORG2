...
pub(super) fn create_schema(conn: &Connection) -> rusqlite::Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS agent_org_execution_final_summary_receipts (
            ...
            FOREIGN KEY(certificate_id)
                REFERENCES agent_org_execution_run_completion_certificates(id)
                ON DELETE CASCADE,
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_org_execution_final_summary_one_active
            ON agent_org_execution_final_summary_receipts(certificate_id)
            WHERE status IN ('pending','running','persisting');
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_final_summary_run_current
            ON agent_org_execution_final_summary_receipts(
                org_run_id,activation_generation,attempt DESC
            );
        CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_org_execution_final_summary_turn
            ON agent_org_execution_final_summary_receipts(
                coordinator_session_id,turn_intent_id
            ) WHERE turn_intent_id IS NOT NULL;
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_final_summary_public_timeline
            ON agent_org_execution_final_summary_receipts(org_run_id,terminal_at,receipt_id)
