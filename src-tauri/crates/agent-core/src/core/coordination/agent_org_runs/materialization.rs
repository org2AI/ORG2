...
pub(super) fn init_schema(conn: &Connection) -> rusqlite::Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS agent_org_execution_member_materializations (
        ...
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_member_materializations_pending
            ON agent_org_execution_member_materializations(status, org_run_id, generation);

        CREATE TABLE IF NOT EXISTS agent_org_execution_initial_inputs (
        ...
        CREATE INDEX IF NOT EXISTS idx_agent_org_execution_initial_inputs_dispatch
            ON agent_org_execution_initial_inputs(status, org_run_id);",
    )
}
...
pub(super) fn load_initial_input_with_connection(
    ...
    conn.query_row(
        "SELECT org_run_id, turn_intent_id, message_id, content, payload_json,
                status, created_at, updated_at
         FROM agent_org_execution_initial_inputs WHERE org_run_id=?1",
        [org_run_id],
...
pub(super) fn load_initial_input_by_turn_with_connection(
    ...
    conn.query_row(
        "SELECT org_run_id, turn_intent_id, message_id, content, payload_json,
                status, created_at, updated_at
         FROM agent_org_execution_initial_inputs WHERE turn_intent_id=?1",
        [turn_intent_id],
        row_to_initial_input,
    )
    .optional()
