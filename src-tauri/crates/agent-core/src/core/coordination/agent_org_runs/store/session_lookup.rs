        ...
        for hop in 0..=MAX_PARENT_WALK_DEPTH {
            if !visited.insert(current_id.clone()) {
                tracing::warn!(
                    ...
                    cycle_at = %current_id,
                    "[agent_org_execution_runs] parent_session_id chain has a cycle; aborting walk"
                );
                return Ok(None);
            }
            ...
            if hop == MAX_PARENT_WALK_DEPTH {
                tracing::warn!(
                    ...
                    "[agent_org_execution_runs] parent_session_id walk exceeded max depth ({}); giving up",
