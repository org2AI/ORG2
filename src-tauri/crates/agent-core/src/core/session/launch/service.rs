//! Session launch application service shared by command adapters and durable replay.
//!
//! Owns launch admission and Rust/CLI dispatch. The serializable request is also
//! the durable launch snapshot contract; command adapters do not own that schema.
...
/// Wire value for `SessionLaunchRequest.category` selecting the Rust-native
...
/// Wire value for `SessionLaunchRequest.category` selecting an external CLI
...
pub struct SessionLaunchRequest {
    ...
    /// omit this; `launch_session` creates and claims the Run before
...
pub async fn launch_session(
    ...
    mut params: SessionLaunchRequest,
...
mod tests {
    ...
    #[test]
    fn launch_snapshot_preserves_the_public_camel_case_contract() {
        let request: super::SessionLaunchRequest = serde_json::from_value(serde_json::json!({
            "category": "rust_agent",
            "content": "hello",
            "workspacePath": "/repo",
            "additional_directories": ["/other"],
            "durableRunId": "wir_saved",
            "agentDefinitionId": "builtin:sde",
            "nativeHarnessType": "codex",
            "mode": "ask"
        }))
        .unwrap();
        assert_eq!(request.additional_directories, vec!["/other"]);
        let encoded = serde_json::to_value(&request).unwrap();
        assert_eq!(encoded["workspacePath"], "/repo");
        assert_eq!(
            encoded["additionalDirectories"],
            serde_json::json!(["/other"])
        );
        assert!(encoded.get("additional_directories").is_none());
        assert_eq!(encoded["durableRunId"], "wir_saved");
        assert_eq!(encoded["nativeHarnessType"], "codex");
        assert_eq!(encoded["mode"], "ask");
        let replay: super::SessionLaunchRequest = serde_json::from_value(encoded).unwrap();
        assert_eq!(replay.durable_run_id.as_deref(), Some("wir_saved"));
        assert_eq!(replay.agent_definition_id.as_deref(), Some("builtin:sde"));
    }
