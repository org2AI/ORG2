...
fn build_agent_org_completion_candidate_snapshot(
    ...
    lines.push(match assessment.state {
        ...
        RunCompletionCandidateState::Certified => "- A current completion certificate already exists, only for its closed work episode. Do not call `org_run_complete` again or rewrite that episode. A new authorized user request opens a new episode through the normal Task creation tools when the Team is Idle; an old certificate does not satisfy that request. Follow any explicit admission blocker without polling.".to_string(),
    });
    lines
}
