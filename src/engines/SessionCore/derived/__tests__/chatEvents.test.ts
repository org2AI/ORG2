...
describe("chatEventsAtom live streaming overlay", () => {
  it("publishes recovered delivery ownership when message text and tail stay unchanged", () => {
    const store = createStore();
    store.set(sessionIdAtom, "session-1");
    const failed = makeChatEvent(
      "queued-user:legacy:",
      "2026-06-06T20:00:00Z",
      {
        ...
        result: {
          ...
          queueMessageId: "legacy",
          message: { role: "user", content: "retry me" },
        },
      }
    );
    const tail = makeChatEvent("error", "2026-06-06T20:00:01Z");
    store.set(derivedSnapshotAtom, makeSnapshot([failed, tail], false));
    const before = store.get(chatEventsAtom);
    const { queueMessageId: _removed, ...recoveredResult } = failed.result;
    const recovered = { ...failed, result: recoveredResult };
    store.set(derivedSnapshotAtom, makeSnapshot([recovered, tail], false));
    const after = store.get(chatEventsAtom);
    expect(after).not.toBe(before);
    expect(after[0].result.queueMessageId).toBeUndefined();
  });
