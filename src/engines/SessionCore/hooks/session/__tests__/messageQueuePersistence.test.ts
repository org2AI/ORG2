...
describe("messageQueuePersistence", () => {
  ...
  it.each(["pending", "failed"])(
    "recovers an ownerless %s projection in place after owner hydration",
  ...
  it.each(["pending", "failed"])(
    "preserves %s rows with any durable owner and accepted provider rows",
    async (deliveryStatus) => {
      const owned = {
  ...
  it.each(["pending", "failed"])(
    "preserves a %s row accepted while durable ownership is being checked",
    async (deliveryStatus) => {
      const pending = {
  ...
  it("keeps a failed background write visible to the commit barrier", async () => {
    const store = createStore();
    await hydrateMessageQueue(store);
    mocks.persist.mockRejectedValueOnce(new Error("disk unavailable"));
    const next = message("failed-write");
    store.set(messageQueueAtom, [next]);

    await expect(flushMessageQueuePersistence(store)).rejects.toThrow(
      "disk unavailable"
    );
    expect(store.get(messageQueueAtom)).toEqual([next]);

    const later = message("later-write");
    store.set(messageQueueAtom, [next, later]);
    await expect(flushMessageQueuePersistence(store)).resolves.toBeUndefined();
    expect(mocks.persist).toHaveBeenLastCalledWith([next, later]);
  });
