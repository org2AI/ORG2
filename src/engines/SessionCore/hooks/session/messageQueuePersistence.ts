...
function installQueuePersistence(store: Store): void {
  ...
  const unsubscribe = store.sub(messageDeliveryRecordsAtom, () => {
    ...
    void persistQueueBestEffort(store).catch((error: unknown) => {
...
function unresolvedQueueMessageId(event: SessionEvent): string | null {
  const queueMessageId = event.result?.queueMessageId;
  const deliveryStatus = event.result?.deliveryStatus;
  if (
    event.source !== "user" ||
    (deliveryStatus !== "pending" && deliveryStatus !== "failed") ||
    event.displayStatus !== deliveryStatus ||
    event.result?.deliveryOwnerRetired === true ||
...
/**
 * Recover legacy pending/failed projections that have no execution owner.
 *
 ...
 * that became sent while the owner was settling. The visible user row
...
export async function reconcileOrphanedOptimisticQueueProjections(
  ...
  const currentOrphans = (await eventStoreProxy.getEvents(sessionId)).filter(
    (event) => {
      const queueMessageId = unresolvedQueueMessageId(event);
      ...
      const result = {
        ...
        deliveryError:
          event.result?.deliveryStatus === "failed"
            ? event.result.deliveryError
            : ORPHANED_QUEUE_DELIVERY_ERROR,
      };
...
function installOrphanProjectionReconciliation(store: Store): void {
  ...
  const inspectSnapshot = (snapshot: Snapshot, sessionId: string) => {
    ...
    const hasCandidate = tail.some((event) => unresolvedQueueMessageId(event));
