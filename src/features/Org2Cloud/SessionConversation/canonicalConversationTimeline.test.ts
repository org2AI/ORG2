  ...
  it("keeps the anchor's failed retry row when a family replay already published its intent", () => {
    const failed = {
      ...
      result: {
        ...
        queueMessageId: "current",
      },
    };
    const timeline = assembleCanonicalConversationTimeline({
      ...common,
      anchorBareSessionId: "fork",
      anchorEvents: [failed],
      eventsByBareSessionId: new Map([["root", [currentTurn]]]),
      viewer: { status: "known", userId: "bob" },
      planeEvents: planeEvents.map((row) => ({
        ...row,
        authorUserId: "bob",
    ...
    expect(timeline[0].id).toBe(failed.id);
    expect(timeline[0].result).toEqual(failed.result);
  });
