...
>({});
const log = createLogger("CloudConversationSource");
...
export function useCloudConversationSource({
  ...
  useEffect(() => {
    ...
    void resolveForkWorkspacePath(importedRemoteRow)
      ...
      .catch((error: unknown) => {
        // A failed probe is not evidence that the source has no workspace.
        // Keep it pending until the existing inventory invalidation retries.
        if (!cancelled) log.error("Local workspace resolution failed", error);
      });
    return () => {
      cancelled = true;
    };
