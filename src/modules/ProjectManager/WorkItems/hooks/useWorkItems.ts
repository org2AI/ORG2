...
export function useWorkItems(options: UseWorkItemsOptions = {}) {
  ...
  const data = useWorkItemsData({
    ...
    projectId: projectData.project?.id ?? projectId ?? null,
    projectSlug,
    statusOrgId: projectData.project?.orgId ?? null,
    onAssignmentChanges,
  ...
  return {
    ...
    data: {
      ...
      useLatestRevisionConflict: data.useLatestRevisionConflict,
      keepMineRevisionConflict: data.keepMineRevisionConflict,
      getShortId: data.getShortId,
