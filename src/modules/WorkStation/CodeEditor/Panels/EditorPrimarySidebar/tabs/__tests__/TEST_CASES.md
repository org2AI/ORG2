...
| 3   | Select a worktree row                        | Breadcrumb shows `{branch}` only (no redundant folder prefix); **one** icon-only loading state with an accessible label appears immediately in the changes area; previous scope files are not visible |
